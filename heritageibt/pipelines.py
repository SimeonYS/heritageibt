...
class HheritageibtPipeline:

    # Database setup
    conn = sqlite3.connect('heritageibt.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `heritageibt`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM heritageibt WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO heritageibt (date, title, link, content)"
